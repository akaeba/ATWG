...
class TestATWG(unittest.TestCase):
    ...
    def test_parse_cli(self):
        ...
        self.assertDictEqual(chamberArg, {'chamber': 'ESPEC_SH641', 'port': None})
    #*****************************
    ...
    def test_open(self):
        ...
        with self.assertRaises(ValueError) as cm:
            dut.open(chamberArg={'chamber': 'unknown', 'port': None}, waveArg={})
        self.assertEqual(str(cm.exception), "Unsupported climate chmaber 'unknown' selected")
        # exception: unsupported waveform
        with self.assertRaises(ValueError) as cm:
            dut.open(chamberArg={'chamber': 'SIM', 'port': None}, waveArg={'ts': 1, 'tp': 3600, 'wave': 'unknown'})
        self.assertEqual(str(cm.exception), "Unsupported waveform 'unknown' requested")
        # open in sim mode with sine wave
        self.assertTrue(dut.open(chamberArg={'chamber': 'SIM', 'port': None}, waveArg={'ts': 1, 'tp': 3600, 'wave': 'sine'}))
    #*****************************
    ...
    def test_stop(self):
        ...
        self.assertTrue(dut.open(chamberArg={'chamber': 'SIM', 'port': None}, waveArg={'ts': 1, 'tp': 3600, 'wave': 'sine', 'initVal': 25}))
    ...
    def test_status(self):
        ...
        # prepare
        dut = ATWG()
        self.assertTrue(dut.open(chamberArg={'chamber': 'SIM', 'port': None}, waveArg={'ts': 1, 'tp': 3600, 'wave': 'sine', 'highVal': 60, 'lowVal': 10, 'initVal': 30}))
        self.assertTrue(dut.start());
